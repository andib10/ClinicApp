...
import com.licenta.MedicalClinic.dto.DoctorDTO;
import com.licenta.MedicalClinic.entity.*;
import com.licenta.MedicalClinic.service.*;
...
public class PatientController {
    ...
    public String deleteAppointment(@RequestParam("appointmentId") int theid, Principal principal){
        ...
        if (appointment == null || appointment.getPatient().getId() != patientt.getId()) {
            ...
            LocalDateTime slotStart = appointment.getSlotStart();

            // Send cancellation email to patient
            String patientEmailContent = emailTemplateService.buildAppointmentCancellationEmailForPatient(patient, doctor, slotStart);
            emailService.sendHtmlEmail(patient.getUser().getEmail(), "Programare anulata", patientEmailContent);

            // Send cancellation email to doctor
            String doctorEmailContent = emailTemplateService.buildAppointmentCancellationEmailForDoctor(doctor, patient, slotStart);
            emailService.sendHtmlEmail(doctor.getUser().getEmail(), "Programare anulata", doctorEmailContent);
...
/*    @ResponseBody
    ...
    public List<Doctor> getDoctorsBySpecialty(@PathVariable("specialtyId") int specialtyId) {
        return doctorService.findAllBySpecialtyId(specialtyId);
    }*/

/*    retrieves a list of doctors by specialty from the database and transforms them into DoctorDTO objects, which include
    the doctor's ID, first name, and last name. It then returns these DTOs in a ResponseEntity, ensuring efficient data transfer
...
//        appointment.setDoctor(new Doctor(doctorId));
        ...
        // Send confirmation email using Thymeleaf template
        String emailContent = emailTemplateService.buildAppointmentConfirmationEmail(patient, doc, appointment.getSlotStart());
        emailService.sendHtmlEmail(patient.getUser().getEmail(), "Programare Confirmata", emailContent);

        // Send confirmation email to doctor
        String doctorEmailContent = emailTemplateService.buildAppointmentNotificationForDoctor(doc, patient, appointment.getSlotStart());
        emailService.sendHtmlEmail(doc.getUser().getEmail(), "Noua Programare Confirmata", doctorEmailContent);
